...
async function runTests() {
  ...
  try {
    ...
    await client.query("UPDATE dune.bot_active_playtime SET last_login_date = CURRENT_DATE - INTERVAL '2 days' WHERE character_id = 123");
    
    // Trigger the function twice
    await client.query("SELECT dune.fn_check_daily_weekly_rewards(1, 123)");
    await client.query("SELECT dune.fn_check_daily_weekly_rewards(1, 123)");
    
    const countRes = await client.query("SELECT COUNT(*) as c FROM dune.bot_pending_deliveries WHERE account_id = 1");
